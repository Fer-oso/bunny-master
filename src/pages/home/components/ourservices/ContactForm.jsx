  ...
  if (isSuccess) {
    return (
      ...
      className={`min-h-screen flex items-center justify-center px-4 py-12 ${ isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"} `}>
        <Alert icon={CheckCircle2} title="¡Gracias!" message="Tu mensaje ha sido enviado con éxito." />
      </div>
    );
  }
...
function Field({
  ...
  return (
    <div className="space-y-2.5">
      <label htmlFor={name} className="text-sm font-semibold text-primary-color flex items-center gap-2">
        {label}
